...
class PLLSimulator():
    ...
    def _grid_phases(self, frequency_offset=0, amplitude_offset=0, init_phase=0, sag=None, jump=None):
        ...
        t = self.t
        sag_gain = torch.ones(n_runs, N)  # Added phase jumps and voltage sags
        if sag is not None:
            t0, duration, depth = sag
            active = (t>=t0) & (t< t0 + duration)
            sag_gain = torch.where(active, depth.expand(n_runs, N), sag_gain)
        phase_jump = torch.zeros(n_runs, N)
        if jump is not None:
            t0, delta = jump
            phase_jump = torch.where(t >= t0, delta.expand(n_runs, N), phase_jump)
            
        noise = noise_amplitude * v_nominal * (torch.rand(3, n_runs, N) - 0.5)
        omega_g = omega_0 + 2 * torch.pi * frequency_offset 
        theta_grid = init_phase + phase_jump
        amplitude = (v_nominal + amplitude_offset) * sag_gain
